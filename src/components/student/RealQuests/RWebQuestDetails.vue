            ...
            <div class="container m-0 rounded mt-2 mb-2 mr-1 p-5" id="CardContainer">
              <div class="col">
                ...
                <ul class="fs-5 text-white">
                  <li>Front-end 👨‍💻:{{ selectedWebQuest.frontend }}</li>
                  <li>Back-end 👩‍🌾:{{ selectedWebQuest.backend }}</li>
                  <li>Ui-Ux 🧑‍🎨:{{ selectedWebQuest.ui_ux }}</li>
                  <li>web_security 🕵️‍♀️:{{ selectedWebQuest.web_security }}</li>
                ...
                <div class="d-flex gap-3">
                  <button type="button" class="btn btn-success btn-rounded mt-3 "
                    ...
                    id="ApplyBtn" @click="applySolo(selectedWebQuest.project_id)">Apply Solo</button>
                  <button type="button" class="btn btn-warning btn-rounded mt-3" data-bs-toggle="modal"
                    @click="applyTeam(selectedWebQuest.project_id)" data-bs-target="#applyModal"
...
export default {
  ...
  methods: {
    ...
    getQuests() {
      ...
      this.selectedWebQuest = this.$store.getters['realQuests/webRQuests'].find((quest) => quest.project_id === toNumm);
    },
