<template>
  <div class="m-0 colored" :key="relaodKey">
        ...
        <div class="row gx-5 justify-content-center">
          <div class="col justify-content-between px-5 mt-5">
            <div class="row justify-content-between">
              <div class="col-6">
                <p class=" fs-2 fw-bold text-white">{{ selectedUiQuest.title }}</p>
            ...
            <div class="container m-0 rounded mt-2 mb-2 mr-1 p-5" id="CardContainer">
              <div class="col">
                <p class="fs-4 text-white">{{ selectedUiQuest.description }}
                </p>
                <p class="fs-4 fw-bold text-white">Required Roles 👨‍💼
                </p>
                <ul class="fs-5 text-white">
                  <li>Front-end 👨‍💻:{{ selectedUiQuest.frontend }}</li>
                  <li>Back-end 👩‍🌾:{{ selectedUiQuest.backend }}</li>
                  <li>Ui-Ux 🧑‍🎨:{{ selectedUiQuest.ui_ux }}</li>
                  <li>web_security 🕵️‍♀️:{{ selectedUiQuest.web_security }}</li>
                </ul>
                  <div class="d-flex gap-3">
                    <button type="button" class="btn btn-success btn-rounded mt-3 "
                      ...
                      id="ApplyBtn" @click="applySolo(selectedUiQuest.project_id)">Apply Solo</button>
                    <button type="button" class="btn btn-warning btn-rounded mt-3" data-bs-toggle="modal"
                      @click="applyTeam(selectedUiQuest.project_id)" data-bs-target="#applyModal"
...
export default {
  ...
  methods: {
    ...
    getTasks() {
      ...
      this.selectedUiQuest = this.$store.getters['realQuests/ui_uxRQuests'].find((quest) => quest.project_id === toNumm);
    },
